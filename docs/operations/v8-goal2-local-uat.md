...
| Authenticated legacy custom fallback | A fresh signed-in owner with no cloud goal completes ordinary custom-role Setup when Research is unavailable or declined. All four workspace pages retain the explicitly labelled legacy/sample experience; absence is distinguished from authentication, storage/network errors, and corrupt Research context. | Pending |
| Authenticated Flagship fallback | A fresh signed-in owner selects Use Flagship and completes the real adaptive Build without a preseeded goal. Only the current setup is activated; device history and any existing immutable cloud plan remain protected. Guest Flagship stays local. | Pending |
...
| Concurrent setup guard | A plan or replacement goal created after the empty preflight snapshot makes the Research setup write conflict atomically; common-role metadata and prior plan remain unchanged, with no false success record. Record real SQLite and local D1 evidence separately. | Pending |
