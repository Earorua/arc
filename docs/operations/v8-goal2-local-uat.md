...
**Latest real attempt (2026-09-06, 09:57 UTC; current status):** Following explicit authorization for exactly one additional Research, the user ran the reviewed diagnostic helper on clean **`7409fbd8c509bc567a68dd35a61551b1e44258d6`** and reported execution. `outputs/live-research/summary-2026-09-06T09-57-33-859Z.json` is **live-one / incomplete / research-failed**: Key **200 / complete / 1,390 ms**, Research **403**, total real requests **2**, Research **1**, Repair **0**. The observer recorded `httpStatus: 403`, `location: http-error`, `errorCode: 403`, `errorCodeState: recognized`, `errorType: null`, `errorTypeState: missing`. Requested Sol is unchanged; actual model/usage are null. Run `1017b0b3-d011-4b17-ad0f-62e8af4d33f2` failed, citations/skills 0, audit 1, local USD 5 reservation released/settled 0, owner/second-request guards true, activation/planning false, database disposed. SHA256: **`eba89578f1ca457974d1d8c45d74b31aea3bae4991e54c71c40d6eeef58c1a87`**. This proves a second failed real attempt and that the optional diagnostic ran; it does not identify the refusal cause or final external charge. Both separate Research allowances are consumed. The user's post-attempt screenshot shows Activity Overview, GMT+8, Past 1 Month, spend **$0.00**, requests **0** and tokens **0**, with no Top API Keys/Top Apps data; this records displayed aggregate values, not a per-request trace or final settlement. Key revocation is unconfirmed. Updated support material is unsent; no further paid attempt, merge, push or deployment occurred. This update only records evidence; prior offline acceptance remains unchanged.

**First real attempt (2026-09-06, 04:23 UTC; historical):** After confirming usable OpenRouter account credit, the user ran `-ExecuteOne` on clean `31f80525c0efe556146f2830b1c049f1055de964`. Report `outputs/live-research/summary-2026-09-06T04-23-53-054Z.json` is **`live-one / incomplete / research-failed`**. Key GET passed (HTTP 200, complete, 2,078 ms); the sole Research POST returned **HTTP 403**, with no Repair or automatic retry. Requested model is `openai/gpt-5.6-sol`; actual model and usage are null. Run `62288a2f-5218-40ae-9c1a-8e33ebd4ac73` failed, with zero citations/skills and one audit. The USD 5 local reservation is released with zero settled micros; owner/second-request rejection checks passed, fresh-account activation and planning did not occur, and the database was disposed. This is evidence of a failed real attempt, **not a passed live validation or proof of no external charge**.

Report SHA-256: `61583f2d3999c2f7c93d92ccd32362602f0628cb9a2a3769c84799048d4819f7`. Only the allowlisted report was inspected; no credentials, environment files or raw provider error were read. HTTP 403 alone does not establish the exact cause. The existing adapter maps it to non-retryable `unavailable` and does not retain a detailed error message. Next collect only the user's non-sensitive error description and charge from the OpenRouter Activity record around **2026-09-06 12:23 Asia/Shanghai**, or confirmation that no record is visible; ask the user to revoke the test key as planned. Any second Research requires diagnosis and fresh explicit authorization. Live acceptance, merge and GitHub backup remain incomplete; production operations and deployment remain excluded. This evidence update changes documentation only; prior code-test results retain their original commit scope.

**Key-only success (2026-09-06):** On clean helper commit `0fb4305cf9b850b91abe692392b78b834513c027`, the user's report `outputs/live-research/summary-2026-09-06T04-02-11-596Z.json` is `key-check-only / passed`, key HTTP 200, phase complete, failure null, elapsed **9,863 ms**. Key GET 1, Research POST **0**, Repair 0, no run/audit/usage/reservation; database disposed. The configured credential policy passed. This validates key preflight only; real Research, model quality/planning integration and GitHub backup remain unperformed. The nearly exhausted 10-second key deadline has been widened to 30 seconds under TDD (3 RED→49 GREEN), two clean independent static reviews and root full regression **135 files / 2,407 tests passed**, 41.82s, plus type/lint/diff checks. The Research 120-second deadline and USD 5/one-POST/no-retry rules remain fixed.

**Second user preflight (2026-09-06):** On clean USD 5 helper commit `9fbf233ed3f9bc7b5d1fc5fc4bc54b4cf7c8a4b5`, report `outputs/live-research/summary-2026-09-06T03-30-05-252Z.json` was `live-one / incomplete / key-check-failed`, with no HTTP status, one GET attempt, Research POST **0**, Repair 0, no run/audit/usage/reservation and disposal true. Approximately 10 seconds elapsed, consistent with the key-check deadline; its exact network cause is unproven. Later unauthenticated public model-metadata checks returned 200 via Node (905 ms) and PowerShell (650 ms). No implementation change or automatic retry was made. Both user attempts remain preflight-only, and the sole Research request remains unconsumed.
...
Final root automated gates and agent-assisted browser UAT: **passed**, with the evidence limits above. Explicit user acceptance of this guided local round: **passed, 2026-09-06**. The feature branch and worktree are retained locally. Two separately authorized real Research attempts returned **403**; live acceptance remains **incomplete**, with no further Research allowance. The next step is provider support, Activity/charge verification and confirmation of test-key revocation. Local integration and GitHub backup remain pending the live acceptance gate. Production configuration/storage changes, Sites candidates and deployment remain prohibited and unperformed.
