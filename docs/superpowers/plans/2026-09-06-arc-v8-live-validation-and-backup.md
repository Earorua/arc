...
**Read-only diagnostic continuation:** The user subsequently found no Activity record and confirmed a personal account with the existing test key not yet revoked. The exclusive `-CheckAccountOnly` helper in `docs/superpowers/plans/2026-09-06-arc-v8-readonly-account-diagnostics.md` is implemented: at most one current-key GET and one user-model-list GET, using the existing masked stdin channel. Final focused tests **175/175**, root full suite **136 files / 2,533 tests**, types, targeted lint and diff checks passed. After one P2 was remediated under TDD, independent specification and quality/security re-reviews both returned **0/0/0, READY**. User-side execution and actual account observations remain pending; the reviewed command is handed off after local save. No Research is authorized by this continuation. The account catalog cannot prove inference success, and key usage is only an aggregate observation.
