...
**Authorized parallel progress (2026-09-06; current sequencing):** The user accepted development-branch GitHub backup followed by local release preparation while waiting on the unresolved 403. Follow `2026-09-06-arc-v8-branch-backup-local-readiness.md`. Ordinary backup of `codex/v8-openrouter-research-beta` may proceed after publication checks without declaring live acceptance; master merge, PR creation, production operations, Sites candidates and deployment are excluded. This supersedes only the previous all-push-after-live sequencing, not any request budget or live acceptance requirement.

**Development-branch backup completed (2026-09-06):** After explicit confirmation of the public repository and full branch source/docs/history, the user answered “允许”. Root ordinarily pushed `codex/v8-openrouter-research-beta` to `https://github.com/Earorua/arc.git`, then independently verified remote/local `70eb49d8842468dab7c1f946cd368742a8da9f90`, unchanged remote master `f6c3cddbe6d3f177f3681b355142daad84f2330e`, and a clean worktree. The branch/commit Actions query returned 0 runs, not a CI pass. The local readiness report and verified complete-history bundle are recorded in the parallel plan. This completion note is subsequently synchronized to the same branch; final handoff verifies the containing commit. Older statements that GitHub backup is incomplete are historical. Real Research acceptance and master integration remain incomplete; no new Provider call, PR, production operation, Sites candidate or deployment occurred.
