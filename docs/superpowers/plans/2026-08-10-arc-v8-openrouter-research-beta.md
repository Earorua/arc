...
- Modify: `app/lib/use-planning-workspace.ts` and its focused tests only if needed for an explicit Research Setup cloud-only read that leaves historical local import data and default behavior intact
- Modify: `app/lib/planning-client.ts` and its focused tests (optional cancellation for the existing bounded workspace read)
- Create: `tests/components/research-setup-activation.test.tsx` (real page/client/route/service/SQLite new-owner activation regression)
- Modify: `app/contracts/cloud-state.ts`, `app/server/cloud/service.ts`, `app/server/cloud/repository.ts`, `app/server/cloud/d1-cloud-repository.ts`, `app/server/http/cloud-route-factories.ts` and focused contract/route/service/SQLite tests (Research-only atomic setup guard; no schema migration or new route)
...
**New-owner activation correction (2026-09-05 quality review):** Final specification review of `82359d2` cleared the previous findings, but quality review found that an eligible authenticated account without an active cloud goal cannot complete Research Build. The common setup hook treats a null cloud snapshot as local; cloud `saveSetup` itself also requires an existing goal. The Research Build path must explicitly persist only its current setup into an owner-bound goal through a bounded existing activation capability, refresh planning, and then generate. Do not import historical device completions/proofs, enqueue Research activation into the shared offline queue, change ordinary legacy import consent, or replace an existing immutable planning workspace. Preserve cancellation and idempotency. Add a real page/client/service/SQLite regression seeded with users only, plus failure/cancellation and local-history isolation evidence. Task 13's new-owner browser scenario must likewise start without a preseeded goal; existing-goal evidence must be labelled separately. This Task 11 finding remains open until repaired and both independent reviews pass.

The Research Setup read may explicitly bypass restoring historical local planning import candidates while leaving them stored and leaving the default planning controller behavior unchanged. An existing immutable cloud planning workspace must be detected before changing its common role metadata. Use the existing bounded activation capability with a fresh current-setup state and conflict rejection; never silently activate an archived goal or import device history to make this scenario pass.

Its cloud-only preflight must use a strict, cancelable workspace GET and allow activation only for a valid empty workspace or an exact `404` / `NOT_FOUND` response. An existing workspace, malformed response, authentication/rate error, or other failure must prevent writes. General offline state is not evidence that no goal exists. Keep owner/lifecycle and operation guards through preflight, activation, refresh, and generation.

**Atomic write correction (2026-09-05 specification re-review):** `8253f58` repairs new-owner activation and passes the initial real SQLite scenarios, but a discriminating concurrent test still fails: another tab can create an immutable plan after the empty preflight snapshot and before common-role save. The Research command must carry a strict optional `intent: "research-setup"` through the existing cloud endpoints; activation with that intent requires empty completion/proof history and conflict rejection. Guard captured owner/active-goal identity and absence of a planning workspace inside the same D1 batch as setup writes, using the existing JSON1 assertion/rollback pattern. Guard both a previously absent goal and an existing empty goal, isolate Research setup idempotency from ordinary setup, and produce stable conflict without changed metadata or a false success record when the guard fails. Do not replace this with another client-side read or relax the guarantee. Default legacy commands remain unchanged. The exact server/contract files above are authorized for this bounded Task 11 remediation; no migration, production operation, or new API route is authorized by it.
...
**User acceptance update (2026-09-06; supersedes the pending-user-acceptance statements in the historical closure/boundary below):** On unchanged code at `976903c105a10e71cf4f73cfd40f05b1b82489d8`, the user explicitly stated “确认本轮本地验收通过” after guided Fake Research→Ready→Build/Path, completion/refresh, Proof submission/withdrawal, account switching with retained A progress, and B Delay Keep/Accept with refresh. Root observed the accepted B schedule: September 6 rest, two tasks on September 7 and 8. The user accepted this local main-flow round; the earlier broader engineering evidence remains separately attributed. This documentation-only update records sign-off in `docs/operations/v8-goal2-local-uat.md` and the resume checkpoint. Full Goal 2 still requires the separately authorized minimal live request and local integration/GitHub backup; no real key/request, production mutation, merge, push, Sites candidate or deployment is authorized by this sign-off.
...
The user's current instruction explicitly excludes live requests and overrides the historical permission question. No key or live authorization was requested. Recorded: “Offline Research Beta complete; no live OpenRouter evidence claimed.” Explicit user acceptance of the guided local round was subsequently recorded on 2026-09-06; Full Goal 2 retains its live-evidence and separately authorized integration/backup gates.
