...
Partial execution evidence (2026-08-01): Google passed hosted sign-in, callback, fresh-navigation session refresh, and sign-out. GitHub passed hosted sign-in, callback, and fresh-navigation session refresh. Both flows use independent Arc. identity and no ChatGPT account. Release-gate review found that the server correctly disabled implicit linking but the account menu did not expose the required authenticated `linkSocial` action. A test-first remediation now lists connected providers, offers only configured unlinked providers, uses same-origin success/error callbacks, presents a generic no-merge conflict state, and settles failed connection reads without exposing provider internals. The focused suite passed 8 tests and the full suite passed 51 files / 244 tests; ESLint, TypeScript, the five-stage Vinext build, rendered HTML, and the client secret-boundary checks passed. GitHub Actions Quality passed the pushed review commit. The identical verified source tree was saved as Sites version 6 with 92 packaged files before the separately approved deployment recorded below. Live cancellation, invalid-state, sign-out, and link-conflict checks are still required before this step can close.

Deployment evidence (2026-08-01): after explicit public-deployment approval, the exact saved version 6 was published to the existing production origin and the deployment reached `succeeded`. A read-only hosted smoke check confirmed the production page title, an active GitHub-backed Arc. session, `GitHub connected`, and the sole eligible `Link Google` action. No linking request or account mutation was triggered during this check. The previous Sites version remains the rollback point; live cancellation, invalid-state, sign-out, and link-conflict checks are still required before this step can close.

Live linking evidence (2026-08-01): after separate action-time approval, the GitHub-authenticated account started `POST /api/auth/link-social` successfully, completed the Google callback, and returned to the safe error URL with `email_doesn't_match`. The current `allowDifferentEmails: false` policy performs this guard before Better Auth reads existing provider ownership, so the expected `account_already_linked_to_different_user` branch was not reached. Arc. showed only the generic no-change recovery message and continued to list GitHub as the sole connected provider. No merge or account mutation occurred. Resolving this requires an explicit security decision: retain same-email-only linking, or design recent-primary-provider reauthentication before cautiously enabling cross-email manual linking.
