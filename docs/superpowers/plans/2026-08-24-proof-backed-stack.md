...
- [x] Write a failing test proving a caller-normalized planning completion yields `practicing`; Task 3 separately tests normalization of legacy completion evidence whose `verified` flag is `true`.
...
- [x] Run `npm run test:unit -- tests/lib/proof/projection.test.ts`; expect a missing-module failure.
- [x] Implement `completedSkillIdsFromPlanning(workspace)` by resolving every `completed` event through its `targetPlanVersionId` and matching Daily Unit `skillId`. Deduplicate repeats and ignore unresolved records rather than inventing evidence.
- [x] Implement latest-version selection by `(proofId, versionNumber)` and apply terminal review events by sequence and timestamp.
- [x] Implement pure projection with the locked precedence and separate `internal` and `public` modes.
...
- [x] Add tests for: structural pass → demonstrated; named validator pass → verified; validator unavailable → demonstrated; validator fail → rejected; rejection and withdrawal downgrade; revision supersedes the prior version; private visibility changes only public projection; remaining evidence prevents an incorrect downgrade; output order follows canonical skill order.
- [x] Add an invariant test that out-of-order review input produces exactly the status generated by the same events in recorded sequence.
- [x] Run `npm run test:unit -- tests/lib/proof/projection.test.ts`; expect all projector tests to pass.
- [x] Stage with `git add app/lib/proof/projection.ts tests/lib/proof/projection.test.ts`.
- [x] Commit with `git commit -m "feat: project deterministic skill evidence"`.
