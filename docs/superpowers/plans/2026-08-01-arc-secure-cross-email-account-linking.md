...
7. [x] exercise a target identity already owned by another Arc. user and confirm the generic no-merge result;
...
Verified-grant expiry evidence (2026-08-04): the user repeated the GitHub source-provider reauthentication, remained on the `Identity verified` screen beyond the five-minute deadline, and then submitted `Continue to Google`. Production rejected the request at `/api/account-link/continue` with a `CONFLICT` response and did not enter Google OAuth. After returning to Arc., the user confirmed the same Arc. account and learning state, `GitHub connected`, and `Link Google`; no provider ownership or product state changed. This verifies the stale-grant expiry subcase of checklist step 8. Because the expired browser credential had already disappeared, the endpoint used the safe generic `Connection wasn't completed. Start again.` response rather than the more specific expiry copy. The raw JSON form-response page is recorded as non-blocking UX debt: a future finish should redirect this expected denial back to `/today?link=expired` without weakening the server check. Replay and direct application bypass remain open, so checklist step 8 stays unchecked.

## Pause and resume checkpoint — 2026-08-04

The user paused before the final approved manual v7 test and will resume when available. Production remains on product release v7.1, Sites version number 8; version 6 remains the rollback artifact. The working branch is `feature/arc-beta-foundation`, with draft PR #3, and the last fully recorded remote commit before this checkpoint is `d99ee82cfeb15690977d7625a755d9eb83b760e0`.

Resume with one controlled target-ownership conflict run: sign into the existing GitHub Arc. user, start `Link Google`, reauthenticate the same GitHub identity, continue within five minutes, and choose the existing Google identity that already owns a separate Arc. user. Expected behavior is a generic no-merge result, `GitHub connected` / `Link Google` on the source user, unchanged source learning state, and the separate Google sign-in still reaching its original Google Arc. user and learning state. Stop immediately on successful linking, account switching, inaccessible identity, or any learning-state change. No result is claimed until the user completes and reports this run.

If the conflict run passes, record checklist step 7, retain steps 5–6 as explicitly deferred because no unowned disposable Google identity is available, preserve replay/application-bypass and observable-log limitations accurately, and close the v7 manual-operation checkpoint before starting product v8 planning and implementation.

Owned-target isolation evidence (2026-08-05): from the existing GitHub Arc. account, the user completed source-provider verification and selected the existing Google identity that already belonged to a separate Arc. account. Production returned the generic `This sign-in method can't be connected to this account.` result. The source session remained the original GitHub Arc. user with `GitHub connected` / `Link Google`, and its learning progress did not change. After signing out, the same Google identity still reached its original Google Arc. user with `Google connected` / `Link GitHub` and unchanged learning progress. No provider ownership, account identity, or learning state moved. This completes checklist step 7. Steps 5–6 remain explicitly deferred because no safe unused Google identity is available; replay/direct application bypass in step 8 and observable sanitized runtime logs in step 9 remain open.

The final Google-account check also exposed a separate device-migration UX defect: a meaningful setup-only local snapshot displayed `0 completed units · 0 proofs`, while **Not now** lived only in component memory and therefore returned after route changes or reloads. Product v7.2 addresses only this prompt behavior. Test-first commits add an account-scoped deterministic snapshot acknowledgement, preserve explicit import consent and all local bytes, explain setup-only work, and make a changed snapshot or different account independently eligible. The release gate passed 61 test files / 516 tests, ESLint, `tsc --noEmit`, the five-stage Vinext production build, and 2/2 rendered-artifact checks. Deployment evidence is recorded only after Sites reports a successful v7.2 deployment.

Product v7.2 completion evidence (2026-08-05): verified GitHub commit `1ac6f2c2414727fef422d94074f7baf43c0a4a5a` was preserved in Sites source commit `7ca5b530dfc58f3cbc700b44a7a881a9bd661209`, saved as Sites version number 9, and publicly deployed successfully as `appgdep_6a728790e0608191bf1286c3a9a3ccfd`. Production returned 200 with `ok` Worker outcomes for `/` and `/api/auth/get-session`; the only error-filtered events were non-fatal `/favicon.ico` 404 responses. The user selected **Not now** on the existing Google account, navigated among Today, Path, Stack, and Proof, reloaded, and confirmed the unchanged prompt remained absent with account identity and learning progress unchanged. This closes v7 stabilization and authorizes transition to the approved v8 product-planning phase. Checklist items 5–6, replay/application-bypass portions of item 8, account-link event-log evidence in item 9, and post-link continuity remain explicitly deferred rather than claimed complete.
