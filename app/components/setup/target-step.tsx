...
import type { AvailabilityVersion, PathBuildResult, PlanningTarget, SkillAuditVersion, UnitRegistry } from "../../contracts/planning";
...
export function TargetStep({ audit, availability, blueprint, registry, planningDate, target, selectedScope, onTargetChange, onScopeChange, buildPaths = buildLearningPaths }: {
  audit: SkillAuditVersion; availability: AvailabilityVersion; blueprint: RoleBlueprint; registry: UnitRegistry; planningDate: string;
  ...
  buildPaths?: (input: PathBuildInput) => PathBuildResult;
}) {
  const targetValid = Number.isInteger(target.targetWeeks) && target.targetWeeks >= 4 && target.targetWeeks <= 52;
  let paths: PathBuildResult | null = null;
  let pathError: string | null = null;
  if (targetValid) {
    try { paths = buildPaths({ blueprint, registry, audit, availability, target, planningDate }); } catch { pathError = "Arc could not compare these paths. Review the inputs and try again."; }
  }
